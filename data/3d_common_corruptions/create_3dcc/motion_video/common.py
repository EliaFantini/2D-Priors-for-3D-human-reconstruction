...
maxxx_depth = 8000.0 / (2**16 - 1)
nrmlz = transforms.Normalize([0.0], [maxxx_depth * 1.0])
downsample = transforms.Resize(256, interpolation=PIL.Image.BILINEAR)
def process_load(npyImage, objSettings, depth_loaded=None):
	...
	use_pred = False #or use GT

	if use_pred:
		## use kenburns predicted depth
		...
		depth_GT = depth_loaded

		tenDepth = depth_GT.cuda().unsqueeze(0).unsqueeze(0)

		my_disparity = 1.0 / (tenDepth + 0.0000001)

		my_disparity = my_disparity / my_disparity.max() * objCommon['fltBaseline']
		tenDepth = (objCommon['fltFocal'] * objCommon['fltBaseline']) / (my_disparity + 0.0000001)
		tenDisparity = my_disparity
...
def process_kenburns(objSettings):
	...
	if 'boolInpaint' not in objSettings or objSettings['boolInpaint'] == True:
		...
		for fltStep in [ 0.0, 0.25, 0.50, 0.75, 1.0 ]: #more inpainting locations
			fltFrom = 1.0 - fltStep
			fltTo = 1.0 - fltFrom
...
def preprocess_kernel(strKernel, objVariables):
	with open('./motion_video/common.cuda', 'r') as objFile:
...
def launch_kernel(strFunction, strKernel):
	if 'CUDA_HOME' not in os.environ:
		...
		os.environ['CUDA_HOME'] = sorted(glob.glob('/ssoft/spack/arvine/v1/opt/spack/linux-rhel7-skylake_avx512/gcc-8.4.0/cuda-11.6.2-aos2hqzylegqm2gwsxkdykq643hql4vf') + glob.glob('/ssoft/spack/arvine/v1/opt/spack/linux-rhel7-skylake_avx512/gcc-8.4.0/cuda-11.6.2-aos2hqzylegqm2gwsxkdykq643hql4vf'))[-1]
	# end
