...
def create_motion_data(BASE_PATH_RGB=None, BASE_PATH_DEPTH=None, BASE_TARGET_PATH=None, BATCH_SIZE=1):
    ...
    for idx, group in enumerate(rgb_and_depth_loader):
        ...
        for iii in tqdm.tqdm(range(len(all_rgb_files))):
            ...
            motion_blur_sev1_path = f"{base_savedir}/xy_motion_blur/{CLASS}/{idx_curr}"

            zoom_blur_sev1_path = f"{base_savedir}/z_motion_blur/{CLASS}/{idx_curr}"
            ...
            intWidth = min(int(1024 * fltRatio), 512)
            intHeight = min(int(1024 / fltRatio), 512)
            ...
            if bool(random.getrandbits(1)):
                random_u = np.random.randint(-75, high=-50)
            else: 
                random_u = np.random.randint(50, high=75)
            
            random_v = np.random.randint(-75, high=75)

            objTo['fltCenterU'] = objFrom['fltCenterU'] + random_u
            objTo['fltCenterV'] = objFrom['fltCenterV'] + random_v
