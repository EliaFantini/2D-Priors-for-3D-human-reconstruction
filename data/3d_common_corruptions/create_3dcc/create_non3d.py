...
def save_corrupted_batches(loader,save_path=None, CLASS=None):
    ...
    for idx, group in enumerate(tqdm.tqdm(loader)):
        ...
        for ii, img in enumerate(rgb_batch):
            ...
            out2, out3 = iso_noise(img, severity=s), low_light(img, severity=s)
            dir_path2 = os.path.join(save_path, 'iso_noise', CLASS)
            dir_path3 = os.path.join(save_path, 'low_light', CLASS)
            filename = paths[ii]
...
def create_non3d_data(BASE_PATH_RGB=None, BASE_TARGET_PATH=None, BATCH_SIZE=1):
    ...
    corruptions_to_generate = ['iso_noise','low_light'] # 'color_quant',
...
def create_non3d_data_low_light(BASE_PATH_RGB=None, BASE_TARGET_PATH=None, BATCH_SIZE=1):
    save_corrupted_batches_low_light(BASE_PATH_RGB, save_path = BASE_TARGET_PATH)

def save_corrupted_batches_low_light(image_loc,save_path=None):
    image = TF.to_tensor(Image.open(image_loc).convert("RGB"))
    img = T.ToPILImage()(image)
    out3 = low_light(img, severity=4)
    out3.save(save_path)

def create_non3d_data_iso_noise(BASE_PATH_RGB=None, BASE_TARGET_PATH=None, BATCH_SIZE=1):
    save_corrupted_batches_iso_noise(BASE_PATH_RGB, save_path = BASE_TARGET_PATH)

def save_corrupted_batches_iso_noise(image_loc,save_path=None):
    image = TF.to_tensor(Image.open(image_loc).convert("RGB"))
    img = T.ToPILImage()(image)
    out2 = iso_noise(img, severity=4)
    out2.save(save_path)